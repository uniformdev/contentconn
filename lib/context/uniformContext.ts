...
class YoloTransitionStore extends TransitionDataStore {
  ...
  constructor() {
    ...
    this.visitorId = id;
    this.client = createClient({ publicApiKey: process.env.NEXT_PUBLIC_LIVEBLOCKS_PUBLIC_KEY });
    ...
    this.room.getStorage().then(storage => {
      this.visitor = storage.root.get('visitor') as LiveObject<Partial<VisitorData>>;
  ...
  async handleUpdateData(_commands: StorageCommands[], computedValue: VisitorData): Promise<void> {
